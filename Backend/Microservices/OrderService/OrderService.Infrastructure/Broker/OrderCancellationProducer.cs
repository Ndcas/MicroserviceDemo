...
internal class OrderCancellationProducer : IOrderCancellationProducer
{
    private IProducer<string> _producer;

    public OrderCancellationProducer(IPulsarClient pulsarClient, IConfiguration configuration)
    {
        var topic = configuration[EnvironmentVariableKeys.OrderCanceledTopic];
