...
internal class OrderCancellationConsumer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var topic = _configuration[EnvironmentVariableKeys.OrderCanceledTopic];
        ...
        await foreach (var message in consumer.Messages(stoppingToken))
        {
            try
            {
                ...
                var response = await productService.PerformStocksUnreservationAsync(guidEventId, objMessage, stoppingToken);
